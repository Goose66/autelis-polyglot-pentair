...
class AutelisNodeServer(SimpleNodeServer):
    ...
    def setup(self):
        ...
        self.threadMonitor = threading.Thread(target=autelisapi.status_listener, args=(ip, self.set_node_state, self.logger))
    ...
    def long_poll(self):
        ...
        if self.threadMonitor and not self.threadMonitor.is_alive():
            ...
            self.logger.warn("Status monitoring thread has terminated - restarting.")
 
            # Restart the monitor thread
            self.threadMonitor = threading.Thread(target=autelisapi.status_listener, args=(self.autelis.controllerAddr, self.set_node_state, self.logger))
            self.threadMonitor.daemon = True
            self.threadMonitor.start()
    ...
    def set_node_state(self, element, value):
        ...
        if element == "runstate":
            retVal = self.get_node("controller").set_driver("GV0", int(value), _ISY_INDEX_UOM)
        elif element == "opmode":
            retVal = self.get_node("controller").set_driver("GV1", int(value), _ISY_INDEX_UOM)
        elif element == "model":
            retVal = True  # ignore
    	elif element == "freeze":
            retVal = self.get_node("controller").set_driver("GV2", int(value), _ISY_INDEX_UOM)
        elif element == "sensor1":
            retVal = self.get_node("controller").set_driver("GV3", int(value), _ISY_INDEX_UOM)
        elif element == "sensor2":
            retVal = self.get_node("controller").set_driver("GV4", int(value), _ISY_INDEX_UOM)
        elif element == "sensor3":
            retVal = self.get_node("controller").set_driver("GV5", int(value), _ISY_INDEX_UOM)
        elif element == "sensor4":
            retVal = True  # ignore
        elif element == "sensor5":
            retVal = True  # ignore
        elif element == "poolht":
            if self.exist_node("poolht"):
                retVal = self.get_node("poolht").set_driver("CLIMD", int(value), _ISY_TEMP_F_UOM) 
        elif element == "poolsp":
            if self.exist_node("poolht"):
                retVal = self.get_node("poolht").set_driver("CLISPH", int(value), _ISY_TEMP_F_UOM) 
        elif element == "pooltemp":
            if self.exist_node("poolht"):
                retVal = retVal or self.get_node("poolht").set_driver("CLITEMP", int(value), _ISY_TEMP_F_UOM)           
        elif element == "spaht":
            if self.exist_node("spaht"):
                retVal = self.get_node("spaht").set_driver("CLIMD", int(value), _ISY_TEMP_F_UOM) 
        elif element == "spasp":
            if self.exist_node("spaht"):
                retVal = self.get_node("spaht").set_driver("CLISPH", int(value), _ISY_TEMP_F_UOM) 
        elif element == "spatemp":
            if self.exist_node("spaht"):
                retVal = self.get_node("spaht").set_driver("CLITEMP", int(value), _ISY_TEMP_F_UOM)
        elif element == "airtemp":
            retVal = self.get_node("controller").set_driver("CLITEMP", int(value), _ISY_TEMP_F_UOM)
            ...
            if self.exist_node(element):
                retVal = self.get_node(element).set_driver("ST", int(value), _ISY_INDEX_UOM)
