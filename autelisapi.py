...
class AutelisInterface:
    ...
    def get_status(self):
        ...
        # Allow timeout errors to be ignored - log and return null
        except requests.exceptions.Timeout as e:
            self._logger.warn("HTTP GET timed out - %s", e.strerror)
            return None
        except:
            raise
        ...
        if response.status_code == requests.codes.ok:
            ...
            self._logger.warn("GET from %s returned unsuccessful status code %d", response.url, response.status_code)
            return None
    ...
    def send_command(self, element, label, value):
        ...
        try:
         ...
         # Allow timeout errors to be ignored - log and return false
        except requests.exceptions.Timeout as e:
            self._logger.warn("HTTP GET in send_command() timed out - %s", e.strerror)
            return False
        except:
            raise 
        ...
        if response.status_code == requests.codes.ok:
            ...
            self._logger.warn("GET to %s returned unsuccessful status code %d", response.url, response.status_code)
            return False
...
def status_listener(controllerAddr, statusUpdateCallback=None, logger=None):
    ...
    # Loop continuously and Listen for broadcast messages
    while True:
